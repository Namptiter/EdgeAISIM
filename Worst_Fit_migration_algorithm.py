...
def my_algorithm(parameters):
    ...
    for service in Service.all():  # Iterate over every service
        ...
        if not service.being_provisioned:  # If service needs to be migrated
            ...
            edge_servers = sorted(
                EdgeServer.all(),
                key=lambda s: ((s.cpu - s.cpu_demand) * (s.memory -
                               s.memory_demand) * (s.disk - s.disk_demand)) ** (1 / 3),
                reverse=True,
            )
            ...
            for edge_server in edge_servers:
                ...
                if edge_server.has_capacity_to_host(service=service):
                    ...
                    if service.server != edge_server:
                        print(
                            f"[STEP {parameters['current_step']}] Migrating {service} From {service.server} to {edge_server}")
